...
static LocalVariable* find_local_variable(LocalVariableContext* local_variables, String name)
{
	for (i64 i = 0; i < local_variables->count; ++i)
	{
...
static b32 analyze_expression(Program* program, ExpressionHandle expression_handle, StackInfo* stack_info)
{
	...
	{
		LocalVariable* var = find_local_variable(stack_info->current_local_variables, expression->identifier);
		if (!var)
		{
			fprintf(stderr, "Undeclared identifier '%.*s'.\n", (i32)expression->identifier.len, expression->identifier.str);
			return false;
		}
...
static b32 analyze_statements(Program* program, Function* function, i64 first_statement, i64 statement_count, StackInfo* stack_info)
{
	for (i64 i = 0; i < statement_count; ++i)
	{
		...
		{
			if (!analyze_expression(program, statement.assignment.expression, stack_info))
			{
				return false;
			}

			String identifier = statement.assignment.identifier;
			LocalVariable* var = find_local_variable(stack_info->current_local_variables, identifier);
			if (var)
			{
		...
		else if (statement.type == StatementType_Reassignment)
		{
			if (!analyze_expression(program, statement.assignment.expression, stack_info))
			{
				return false;
		...
		else if (statement.type == StatementType_Block)
		{
			...
			if (!analyze_statements(program, function, j + 1, statement.block.statement_count, stack_info))
			{
				return false;
			}
...
static b32 analyze_function(Program* program, Function* function, LocalVariableContext* local_variable_context)
{
	...
	if (analyze_statements(program, function, function->first_statement, function->statement_count, &stack_info))
	{
